...
# --- 3. ESTILO CSS (IDÊNTICO À IMAGEM) ---
st.markdown("""
    <style>
...
with h3: st.markdown("<p style='text-align:right; color:#8b949e;'>Analises diárias resttante: Ilimitado</p>", unsafe_allow_html=True)

df = get_live_data(ativo)

if df is not None:
    # Dados escalares para evitar ValueError
    ...
    with col_esq:
        ...
        m1, m2 = st.columns(2)
        with m1:
            st.markdown(f"""<div class='card-pro'><b>Informações de ativo</b><br><br>
            Ativo: <span style='float:right'>1.08500</span><br>
        ...
        with m2:
            ...
            medo_fig = go.Figure(go.Indicator(
                ...
                domain = {'x': [0, 1], 'y': [0, 1]},
                title = {'text': "Índice de medro", 'font': {'size': 14}},
                gauge = {
    ...
    with col_dir:
        st.markdown("### Analísie com I.A.")
        p1, p2 = st.columns(2)
        p1.markdown("<div class='pct-up'><h3>68%</h3>Cima</div>", unsafe_allow_html=True)
        p2.markdown("<div class='pct-down'><h3>32%</h3>Bawo</div>", unsafe_allow_html=True)
        ...
        if st.button("ANALISAR ENTRADA AGORA"):
            with st.spinner('Analisando...'):
                ...
                <div class='sinal-box'>
                    ...
                    <small>ATIVO: {ativo[:7]}</small><br>
                    <b>Confinnda: 95% | Inicio: {datetime.now().strftime('%H:%M')}</b><br>
                    <small>gale taz - 35-1.3-4 mu</small>
                </div>
                """, unsafe_allow_html=True)

        st.markdown("""<div style='margin-top:20px'>
            <b>Explização de analisie</b><br>
            <small style='color:#8b949e'>Cruzastano de EMA para upba na SMA (sinal de comgsa).</small>
