...
# --- ÁREA DINÂMICA (ATUALIZA SOZINHA) ---
# O fragment faz com que apenas essa parte do código rode a cada 5 segundos
@st.fragment(run_every=5)
def dashboard_dinamico():
    ...
    with c1:
        ...
        with col_info:
            ...
            preco_fake = 1.187000 + random.uniform(-0.0005, 0.0005)
            st.markdown(f"""
                <div class='card'>
                    <b>Informações do ativo</b><br>
                    <small>
                    Ativo: {ativo_selecionado}<br>
                    Cotação: {preco_fake:.6f}<br>
                    Fundo: 1.186285<br>
                    Topo: 1.190185
                    </small>
                </div>
        ...
        with col_medo:
            ...
            valor_medo = random.randint(45, 60)
            fig = go.Figure(go.Indicator(mode="gauge+number", value=valor_medo, gauge={'axis':{'range':[0,100]}, 'bar':{'color':"yellow"}, 'steps':[{'range':[0,40],'color':"red"},{'range':[40,60],'color':"orange"},{'range':[60,100],'color':"green"}]}))
    ...
    with c2:
        ...
        # Porcentagens mudando
        p_subir = random.randint(30, 70)
        p_descer = 100 - p_subir
        p_cima, p_baixo = st.columns(2)
        p_cima.markdown(f"<div style='background:#1b4332; padding:10px; text-align:center; border-radius:5px; color:#00ff00;'>{p_subir}%<br>Cima</div>", unsafe_allow_html=True)
        p_baixo.markdown(f"<div style='background:#432818; padding:10px; text-align:center; border-radius:5px; color:#ff4b4b;'>{p_descer}%<br>Baixo</div>", unsafe_allow_html=True)
...
# Rodapé de Notícias (Estatístico)
st.markdown("---")
